...
This project serves as a practical implementation to track my learning journey of integrating Machine Learning models into production-ready web APIs. It exposes a Scikit-Learn Random Forest model via **FastAPI**, features robust input validation using **Pydantic**, and provides an interactive web interface built with **Streamlit**. The application is containerized and deployed on an **AWS EC2** machine using **Docker**.
...
* **Interactive Web UI**: Includes a Streamlit frontend (`frontend.py`) for users to enter details and receive real-time premium predictions.
* **Cloud Deployment**: Containerized with Docker and deployed to an AWS EC2 instance using a `docker pull` workflow.
...
* **Containerization & Deployment:** [Docker](https://www.docker.com/) and AWS EC2.
...
* Docker (for containerized setup/deployment)


* [uv](https://github.com/astral-sh/uv?utm_source=gemini) (recommended) or standard `pip` for dependency management.
...
*(Note: The dependencies include `fastapi`, `pandas`, `pydantic`, `scikit-learn`, `streamlit`, and `uvicorn`)*.
...
To start the Streamlit UI, open a second terminal and run:
...
### 2. Docker & AWS EC2 Deployment

This application is configured for deployment on an AWS EC2 instance using Docker.

**Build and Push the Image (Locally):**

```bash
docker build -t <your-dockerhub-username>/ml-fastapi .
docker push <your-dockerhub-username>/ml-fastapi
...
SSH into your EC2 instance and pull the Docker image to run the API:

```bash
docker pull <your-dockerhub-username>/ml-fastapi
docker run -d -p 8000:8000 <your-dockerhub-username>/ml-fastapi

```

*(Note: The frontend is configured to target the EC2 API endpoint at `http://16.178.42.100:8000/predict`)*.
